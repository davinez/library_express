  ...
  async.parallel(
    ...
    (err, results) => {
      ...
      if (results.genre == null) {
        // No results.
        const errNotFound = new Error('Genre not found');
...
exports.genre_create_post = [
  /*
  1- Defines a body validator
  2- Validates and santizes the name field.
  3- We check for data already in DB to avoid duplicates
  */
  ...
  async.parallel(
    ...
    (err, results) => {
      ...
      res.render('genre_delete', {
        ...
        genre: results.genre,
        genre_books: results.genre_books,
        genre_books_length: results.genre_books.length,
      });
    }
  );
};
...
exports.genre_update_get = function (req, res, next) {
  Genre.findById(req.params.id).exec((err, genre) => {
    ...
    // Genre not found
    if (genre == null) {
      const errGenreNotFound = new Error('Genre not found');
      errGenreNotFound.status = 404;
      return next(errGenreNotFound);
    }
    // Successful, so render
    res.render('genre_form', {
      title: 'Update Genre',
...
exports.genre_update_post = [
  // Validates and santizes the name field.
  body('name', 'Genre name required').trim().isLength({ min: 1 }).escape(),
